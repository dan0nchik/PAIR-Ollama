...
def load_attack_and_target_models(args):
    ...
    targetLM = TargetLM(
        ...
        use_ollama=(
            True
            if args.target_model in [model.value for model in OLLAMA_MODEL_NAMES.keys()]
            else False
        ),
    )
...
def load_indiv_model(
    model_name, local=False, use_jailbreakbench=False, use_ollama=True
):
    use_jailbreakbench = False
    print(f"Loading model {model_name} with use_ollama={use_ollama}")
...
class AttackLM:
    ...
    def preprocess_conversation(self, convs_list: list, prompts_list: list[str]):
        ...
        if self.initialize_output:
            ...
            if (
                len(convs_list[0].messages) == 0
            ):  # If is first message, don't need improvement
...
class TargetLM:
    ...
    def __init__(
        ...
        self.model = load_indiv_model(
            model_name, evaluate_locally, use_jailbreakbench, use_ollama=True
        )
        self.category = category
        self.template = ""
